...
const createSaleSchema = z.object({
  ...
  saleType: z.enum(["MESA", "AVULSA"]).optional(),
  customerName: z.string().trim().min(1).nullable().optional(),
  mesaId: z.string().trim().min(1).nullable().optional(),
...
async function resolveTenantId() {
  ...
  if (!typedMemberships || typedMemberships.length === 0) {
    return {
      error: NextResponse.json({ error: "Usuario sem membership" }, { status: 403 }),
    };
  }
...
export async function POST(request: Request) {
  ...
  if (userRole === "USUARIO") {
    return NextResponse.json(
      { error: "Sem permissao para registrar venda" },
      { status: 403 },
    );
  }
  ...
  const payload = {
    ...
    sale_type: parsed.data.saleType ?? "AVULSA",
    customer_name: parsed.data.customerName ?? null,
    mesa_id: parsed.data.mesaId ?? null,
